...
-- Create the HSLootMaster Addon
local HawkShadowLootMaster = LibStub("AceAddon-3.0"):NewAddon("HawkShadowLootMaster");

function HawkShadowLootMaster:OnInitialize()
    self.db = LibStub("AceDB-3.0"):New("HawkShadowLootMasterDB");
end

function HawkShadowLootMaster:OnEnable()
    -- Do more initialization here, that really enables the use of your addon.
    -- Register Events, Hook functions, Create Frames, Get information from 
    -- the game that wasn't available in OnInitialize
end

function HawkShadowLootMaster:OnDisable()
...
function HSLootMaster:GetOrCreate&&&&&&&&&&&&&&PlayerInfos(unit)
